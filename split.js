...
/*
Usage:
$ cat listfile | TZ=UTC node split.js input.mp4 {out_dir=.} {start_index=1} {ext=.mp4}
...
var argv = {
  ...
  startIdx: Number(process.argv[4]) || 1,
  fileExt: process.argv[5] || ".mp4",
};
...
function main() {
  ...
  for (var i = 0, n = plist.length; i < n; ++i) {
    var outFile = path.join(argv.outDir, `${leadzero(i + argv.startIdx, n)} ${normFileName(plist[i].name)}${argv.fileExt}`);
