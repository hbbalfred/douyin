...
async function main() {
  ...
  for(;;) {
    verbose("Start to sniffing channels:", channels.length);
    const newVideos = await sniff(channels);

    if (newVideos.length > 0) {
      verbose("Found new videos:", newVideos.length);
      newVideos.forEach(video => verbose("Video:", youtube.makeWatchingUrl(video.videoId)));
...
async function loadVideosFromChannel(channelUrl) {
  const pageSource = await network.loadData(channelUrl);
