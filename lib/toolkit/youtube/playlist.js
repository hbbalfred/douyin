...
async function main() {
  ...
  let estimatePreprocessTime = 10; // seconds
  ...
  for (let i = 0, n = playlist.length; i < n; ++i) {
    ...
    const filename = ds.leadZero(_get(playlist[i], "title.simpleText"), i, n);
    const quality = argv.quality;
    const sampleRate = argv.sampleRate;
    await download({ videoId, outDir, filename, quality, sampleRate, });

    timer = process.hrtime(timer);
    estimatePreprocessTime = (estimatePreprocessTime + timer[0]) * 0.5;

    verbose(
      "Downloaded", filename,
      "Used", timer[0], "seconds.",
      "Remain", (n - i - 1), "video(s),",
      "estimate", (estimatePreprocessTime * (n - i - 1) / 60).toFixed(2), "minutes"
    );
  }
