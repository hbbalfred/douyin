...
const argv = require("yargs")
  ...
  .option("filename", {
    type: "string",
    description: "The file name to saved.",
    default: "final.mp4",
  })
  .option("verbose", {
    default: true
...
const FINAL_FILE = path.join(OUTPUT_DIR, argv.filename);
...
async function main() {
  ...
  if (!info.adaptive_fmts) {
    ...
    const pageSource = await network.loadData(watchingUrl);
...
async function downloadThumbnail(videoId, resolution, image) {
  ...
  await network.loadDataTo(url, dest);
}
...
async function downloadVideo(adaptive_fmts) {
  ...
  await network.loadDataTo(video.url, VIDEO_FILE, { onProgress: wrapOnProgress(logger, "Video loading") });
  logger.flush();
}
...
async function downloadAudio(adaptive_fmts) {
  ...
  const logger = sys.singleLineLog();
  await network.loadDataTo(audio.url, AUDIO_FILE, { onProgress: wrapOnProgress(logger, "Audio loading") });
  logger.flush();
}
