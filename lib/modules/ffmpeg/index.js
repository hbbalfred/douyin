...
ffmpeg.convert = function convert(sourceFile, destFile, options = {}) {
  ...
  // "-ac 2" means stereo
  const command = `ffmpeg -v error -i "${sourceFile}" -y -c:a ${audio_encode} -c:v ${video_encode} -r ${fps} -b:v ${bit_rate} -ac 2 "${destFile}"`;
  return executeCommand(command);
};
...
ffmpeg.speed2x = function speed2x(sourceFile, destFile) {
  const command = `ffmpeg -y -i "${sourceFile}" -filter_complex "[0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a]" -map "[v]" -map "[a]" "${destFile}"`;
  return executeCommand(command);
};
...
ffmpeg.merge = function merge(videoFile, audioFile, destFile) {
  const command = `ffmpeg -y -i "${audioFile}" -i "${videoFile}" -c copy "${destFile}"`;
  return executeCommand(command);
};
