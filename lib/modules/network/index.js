...
network.loadDataTo = function loadDataTo(url, destFile, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    request(requestOptions)
      ...
      .on("complete", () => process.nextTick(resolve))
...
function makeLoadOptions(uri, mimeType, options) {
  ...
  let withoutProxy = (uri.startsWith("https://") && process.env.https_proxy)
    || (uri.startsWith("http://") && process.env.http_proxy);

  return {
    uri,
    method: "GET",
    proxy: withoutProxy ? undefined : options.proxy,
    headers: {
      "accept": mimeType,
